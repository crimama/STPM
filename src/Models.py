...
class Vgg16(torch.nn.Module):
    def __init__(self, pretrain):
        ...
        features = list(vgg16(weights=True).features)
...
def get_networks(call_model):
    if call_model == 'MKD':
        ...
        teacher = Vgg16(pretrain).cuda()
        student = make_arch(config_type, cfg, use_bias, True).cuda()

        return student,teacher 
    else:
        student = ResNet18(Pretrained=False).cuda()
